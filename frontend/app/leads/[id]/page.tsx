...
export default function LeadDetailPage() {
  ...
  const handleViewJourney = async () => {
    ...
    try {
      const data = await api.getLeadJourney(leadId);
      setJourneyData(data);
    } catch (error) {
      console.error('Failed to load journey:', error);
      toast.error('Failed to load lead journey');
        ...
        <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-5 hover:shadow-lg transition-shadow duration-200">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <button
              ...
              className={`flex items-center justify-center gap-2 px-4 py-3.5 rounded-xl font-semibold transition-all duration-200 text-sm shadow-sm ${
            ...
            <button
              onClick={() => router.push(`/chat/${leadId}`)}
              className="flex items-center justify-center gap-2 px-4 py-3.5 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-xl font-semibold hover:from-blue-600 hover:to-blue-700 transition-all duration-200 text-sm shadow-sm hover:shadow-md active:scale-95"
            >
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-3.5 bg-gradient-to-r from-purple-600 via-purple-700 to-indigo-700 text-white rounded-xl font-semibold hover:from-purple-700 hover:via-purple-800 hover:to-indigo-800 transition-all duration-200 text-sm shadow-md hover:shadow-lg active:scale-95"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
            </svg>
            View Lead Journey Timeline
          </button>
        </div>
